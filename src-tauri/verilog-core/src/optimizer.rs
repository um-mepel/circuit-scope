...
fn expr_size(expr: &IrExpr) -> usize {
    match expr {
        ...
        IrExpr::Unary { operand, .. } => 1 + expr_size(operand),
...
fn expr_references(expr: &IrExpr, name: &str) -> bool {
    match expr {
        ...
        IrExpr::Unary { operand, .. } => expr_references(operand, name),
        IrExpr::Ternary { cond, then_expr, else_expr } => {
...
fn expr_depth(expr: &IrExpr) -> usize {
    match expr {
        ...
        IrExpr::Unary { operand, .. } => 1 + expr_depth(operand),
        IrExpr::Ternary { cond, then_expr, else_expr } => {
...
fn collect_idents(expr: &IrExpr, set: &mut HashSet<String>) {
    match expr {
        ...
        IrExpr::Unary { operand, .. } => { collect_idents(operand, set); }
        IrExpr::Ternary { cond, then_expr, else_expr } => {
...
fn infer_domain(expr: &IrExpr, map: &HashMap<String, ValueDomain>) -> ValueDomain {
    match expr {
        ...
        IrExpr::Unary { operand, .. } => infer_domain(operand, map),
        IrExpr::Ternary { cond, then_expr, else_expr } => {
